...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (highlight.username.toLowerCase() !== player.username.toLowerCase()) {
      sendJson(res, 403, {
        ok: false,
        message: 'You can only view clips for your signed-in profile.'
      })
      return
    }

    if (!highlight.clipSheetUrl) {
      sendJson(res, 404, {
        ok: false,
        message: 'This highlight does not have a replay clip.'
      })
      return
    }

    const blobResponse = await fetch(highlight.clipSheetUrl, {
      headers: {
    ...
    if (!blobResponse.ok) {
      sendJson(res, blobResponse.status, {
        ok: false,
        message: 'Could not load the highlight replay.'
      })
      return
    }

    const contentType = blobResponse.headers.get('content-type') || highlight.clipSheetContentType || 'image/jpeg'
    const clipBuffer = Buffer.from(await blobResponse.arrayBuffer())
    ...
    sendJson(res, statusCode, {
      ok: false,
      message: error.message || 'Failed to load highlight replay.'
    })
  }
}
